 ...
 */
    ...
    public static String sha1(MultipartFile file) {
        ...
        try {
            hashString = sha1(file.getInputStream());
        } catch (Exception e) {
            logger.warn(file.getName() + "文件sha1值计算失败");
        }
        return hashString;
    }
    ...
    public static String sha1(File file) {
        ...
        String hashString = null;
        try {
            hashString = sha1(new FileInputStream(file));
        } catch (Exception e) {
            logger.warn(file.getName() + "文件sha1值计算失败");
        }
        return hashString;
     ...
     * 计算文件sha1值
     */
    public static String sha1(InputStream file) throws IOException {
        String hashString = null;
        byte[] digest = Digests.sha1(file);
    ...
    public static boolean isImage(String filepath) {
        String parentPath;
        if (StringUtils.isBlank(Const.getUserfilesBaseDir())) {
            parentPath = SpringContextHolder.getRootRealPath();
        } else {
            parentPath = Const.getUserfilesBaseDir();
        }
        parentPath += Const.USERFILES_BASE_URL;
    ...
    public static boolean existsFile(String filePath) {
        /*if (!StringUtils.startsWith(filePath, SpringContextHolder.getRootRealPath())) {
            filePath = SpringContextHolder.getRootRealPath() + filePath.replaceAll("^/?", "/");
        }*/
     ...
     * @return 如果存在，则返回true，否则返回false
     */
    public static boolean existsViewFile(String filePath) {
        return FileUtils.existsFile("/templates/modules/" + filePath);
    }
}
