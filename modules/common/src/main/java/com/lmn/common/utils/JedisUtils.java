...
public class JedisUtils {
    ...
    public static final String KEY_PREFIX = Const.getConfig("redis.keyPrefix");
    ...
    static {
        ...
        RedisProperties.Pool props = lettuce.getPool();
        if (props == null) {
        ...
        jedisPool = new JedisPool(config, redisProperties.getHost(), redisProperties.getPort(), redisProperties.getTimeout().getNano(), redisProperties.getPassword());
    }
...
//			logger.debug("getResource.", jedis);
     ...
     * @return
     */
    public static Long geoadd(String key, double longitude, double latitude, String member, int cacheSeconds) {
        Long result = null;
        Jedis jedis = null;
        try {
            jedis = getResource();
            ...
            logger.debug("geoadd longitude={},latitude={} member={}", key, longitude, latitude, member);
        } catch (Exception e) {
            logger.warn("geoadd longitude={},latitude={} member={} e:{}", key, longitude, latitude, member, e);
        } finally {
            returnResource(jedis);
        }
        return result;
    ...
    public static List<GeoRadiusResponse> georadius(String key, double longitude, double latitude, double radius, GeoUnit unit) {
        ...
        try {
            jedis = getResource();
            result = jedis.georadius(getBytesKey(key), longitude, latitude, radius, unit, GeoRadiusParam.geoRadiusParam().withDist());

            logger.debug("getGeoMember {}", key);
        } catch (Exception e) {
            logger.warn("getGeoMember {}", key, e);
        } finally {
            returnResource(jedis);
        }
        return result;
