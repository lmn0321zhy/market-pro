...
public class User extends DataEntity<User> implements IUser {
    ...
    public String getOfficeId() {
        return office != null && office.getId() != null ? office.getId() : "0";
    }
    ...
    public boolean isRootRole() {
        return isRootRole(this.roleList);
    }

    public static boolean isRootRole(List<Role> roleList) {
        return !Collections3.isEmpty(Collections2.filter(roleList, new Predicate<Role>() {
            @Override
            public boolean apply(Role input) {
                return Const.ROOT_ROLE.equals(input.getEnname());
            }
        }));
    }
